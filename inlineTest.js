...
var inTestConfig = {
    ...
    isFalsy: function isFalsy(val) {
        return val === false ||
            ...
            val === -0 ||
            val === '' ||
            val === NaN ||  // V8 doesn't seem to think this is falsy!
            val === null ||
            val === undefined;
    },
    ...
    checkFalsy: function checkFalsy(testName, theTest) {
        if(!this.isTesting) {return;}
        this.testName = testName;
        this.totalTests += 1;
    ...
    checkTruthy: function checkTruthy(testName, theTest) {
        if(!this.isTesting) {return;}
        this.testName = testName;
        this.totalTests += 1;
...
function main() {
    ...
    itc.checkEq('FAIL: bogus test', 2*3, 14);
    itc.checkNotEq('FAIL: 14 does eq 14', 14, 14);
    itc.checkNotExist('FAIL: {} should exist', {});
    itc.checkFalsy('FAIL: {} is not falsy', {});
    itc.checkTruthy('FAIL: 0 is not truthy', 0);
